...
from alert_dedup import already_alerted, mark_alerted
...
def process_dataframe(
    ...
    for i in range(min_bars, len(df)):
        ...
        # CRITICAL: only ever alert on the newest (current) bar. Without this
        # check, every scan re-walks the whole lookback window and re-sends
        # an alert for every bar that historically matched — each carrying
        # THAT bar's entry/sl/tp, not current price. This is what caused
        # live alerts to show levels far behind the current market. The
        # execution path below already had this guard (`i == last_i`);
        # alerting did not, which was the bug.
        alerted = False
        if fired and alert and i == last_i:
            if already_alerted(pair, timeframe, bar_ts):
                print(
                    f"  [dedup] {pair} {timeframe} bar {bar_ts} already alerted — skipping resend"
                )
            else:
                ...
                # Mark as seen regardless of Telegram delivery outcome
                # (including console-fallback mode with no credentials
                # configured) — the goal is to never re-surface the same
                # bar's signal on a later scan, not just to avoid duplicate
                # Telegram deliveries specifically.
                mark_alerted(pair, timeframe, bar_ts)
...
def replay_recent(
    ...
    `errors`, if given, collects {"pair", "timeframe", "reason"} dicts for
    any pair whose live fetch failed. Callers (main() / the API's
    _run_scan) surface these instead of letting a failed fetch look
    identical to "ran fine, nothing to report" — a scan that silently did
    nothing for a pair is exactly how that pair's signals go quietly stale
    while every other pair keeps updating.
    """
    ...
    try:
        ...
        if errors is not None:
            errors.append({"pair": pair, "timeframe": timeframe, "reason": reason})
        if alert:
            # CRITICAL: never let fabricated random-walk data reach an
            # alerting run. Synthetic data is anchored to hardcoded
            # "typical" prices (see data/instruments.py) that can be far
            # from the current real market — silently trading/alerting off
            # it is exactly how live levels end up detached from reality.
            # Synthetic fallback is only safe for pure backtest/ML-bootstrap
            # runs where alert=False.
            print(
                f"  Live fetch failed ({e.__class__.__name__}) for {pair} — "
                f"refusing synthetic fallback because alert=True. No signals "
                f"will be produced this cycle. Check network/yfinance access "
                f"(see DOCKER.md: yfinance rate limits)."
            )
            return []
        print(f"  Live fetch failed ({e.__class__.__name__}) — synthetic fallback (backtest-only, alert=False)")
...
def main():
    ...
    if args.mode == "replay":
        if args.all_pairs or not args.pair:
            ...
            pairs_attempted.append(args.pair)
        ...
        for p in pairs:
            for tf in timeframes:
                pairs_attempted.append(p)
                all_sigs.extend(
                    live_once(p, tf, threshold=args.threshold, executor=executor, errors=fetch_errors)
                )
        all_fired = all_sigs
        print(f"Live pass complete — {len(all_sigs)} new alerts.")

    # Machine-readable summary for the API layer (_run_scan in api/main.py)
    # to parse out of stdout. A scan that fetched nothing for every pair
    # still exits 0 (it didn't crash) — without this, that looked
    # identical to a normal quiet cycle. fetch_errors makes per-pair data
    # failures visible instead of silently vanishing into "0 signals".
    print(
        "\nRUNNER_SUMMARY:"
        + json.dumps(
            {
