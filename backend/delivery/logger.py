...
def log_signal(
    ...
    *,
    ...
    """Append one signal row. Returns the row id.

    Deduped on (pair, timeframe, bar_time): a scan re-evaluates the whole
    lookback window every cycle (auto-scan is hourly by default, replay
    windows overlap, live mode re-walks the same ~120-bar lookback), so the
    *same* historical bar gets re-detected and re-scored on every pass.
    Without this dedup, every cycle appended a brand-new row for every bar
    still inside the window — the log grew without bound, and worse: those
    duplicate rows kept the bar's original (increasingly stale) entry/sl/tp
    alive and re-surfaced as "new" in the feed, which is exactly the
    "outdated stop loss / take profit" symptom for pairs whose patterns
    fire repeatedly within a single lookback window. Now the same bar is
    updated in place instead of re-inserted, so its levels/outcome always
    reflect the most recent evaluation and it appears exactly once.
    """
    global _next_id
    init_db()
    ...
    with _lock:
        ...
        existing = next(
            (
                ...
                and str(r.get("bar_time")) == bar_time
            ),
            None,
        )
        if existing is not None:
            sid = int(existing.get("id", 0))
            existing.update({
                "ts": now,
                "pattern": pattern,
                ...
                "outcome": outcome,
                # Once alerted, stays alerted — don't let a re-evaluation
                # (which never re-alerts, see alert_dedup) flip it back.
                "alerted": bool(alerted) or bool(existing.get("alerted")),
                "meta": meta,
                "updated_at": now,
            })
            _rewrite(rows)
