...
import type { AutoScanStatus, LiveCommand, MlStatus, Order, Signal } from "./types";
...
interface DeskStore {
  ...
  /** True while the desk is showing the synthetic offline feed. */
  usingDemoData: boolean;
  /** epoch ms of the last successful backend refresh, null if never. */
  ...
  setAutoScanning: (v: boolean) => void;
  setCandlesLoading: (v: boolean) => void;
}
...
export const useDeskStore = create<DeskStore>((set, get) => ({
  ...
  setAutoScanning: (v: boolean) => set({ autoScanning: v }),
  setCandlesLoading: (v: boolean) => set({ candlesLoading: v }),

  refreshFromBackend: async () => {
    try {
      const [live, ml, orders, logs, kill, health] = await Promise.all([
      ...
      const auto = (health as any)?.auto_scan as AutoScanStatus | undefined;
      const mlMerged: MlStatus = { ...STAGE4_ML, ...ml };
      // An empty backend result means the engine genuinely found nothing —
      // show that honestly. The old code swapped in demo-data signals here,
      // so a quiet backend looked identical to a busy one and the desk
      // displayed fabricated setups as if they were live.
      set({
        live,
        ml: mlMerged,
        ...
        autoScan: auto ?? get().autoScan,
        autoScanning: !!(auto?.running),
        feed: get().feed + 1,
      });
      ...
      set({ backendOnline: false, usingDemoData: true });
    }
  },
  ...
  scan: async () => {
    if (get().scanning || get().autoScanning) return;
    // No ML health gate. Scanning is what produces closed outcomes, and closed
    // outcomes are the labels the model learns from — refusing to scan because
    // the model looks overfit removes the only thing that would fix it.
    set({ scanning: true });
